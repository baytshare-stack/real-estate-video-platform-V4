  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 text-white">
      <StudioAdsPageHeader
        segment="ads"
        title="Video ads"
        subtitle="Promote your listings with pre-roll and mid-roll creatives. Global company ads are managed in Admin → Ads."
      />
      ...
      <div className="mt-8 space-y-8">
        <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
          ...
          <p className="mt-1 text-xs text-white/50">
            Link a campaign (billing), your ad MP4 URL, slot, and optionally a listing id to target one video only.
            Leave listing id empty to run on all of your channel’s videos.
          </p>
          ...
          <button
            type="button"
            disabled={busy || !campaignId.trim() || !videoUrl.trim()}
            onClick={() => void submit()}
            ...
            <ul className="mt-3 space-y-2 text-sm">
              {ads.map((a) => (
                <li key={a.id} className="rounded-xl border border-white/10 bg-black/25 px-3 py-3">
                  <p className="truncate font-mono text-[11px] text-white/45">{a.id}</p>
                  <p className="truncate text-white/90">{a.videoUrl}</p>
                  <p className="mt-1 text-xs text-white/55">
                    {a.type} · {a.skippable ? `skip ${a.skipAfterSeconds}s` : "non-skippable"} ·{" "}
