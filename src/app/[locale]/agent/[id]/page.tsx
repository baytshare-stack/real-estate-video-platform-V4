...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const user = await getAgentProfile(id);
  if (!user) {
    return pageMetadata(resolved, `/agent/${id}`, {
      title: resolved === "ar" ? "الوكيل غير موجود" : "Agent not found",
      noIndex: true,
    });
  }
  const name = discoverDisplayName(user);
  return pageMetadata(resolved, `/agent/${id}`, {
    title: `${name} | ${resolved === "ar" ? "وكيل عقاري" : "Agent"}`,
    description:
      user.profile?.bio ??
      (resolved === "ar" ? "وكيل عقاري على ريال إستيت تي في" : "Real estate agent on Real Estate TV"),
    ogImage: discoverAvatarUrl(user) ?? null,
  });
}
...
export default async function AgentProfilePage({ params }: Props) {
  ...
  const bio = user.profile?.bio?.trim() || "This agent showcases properties on BytakTube.";
  const phone =
    user.phone?.trim() ||
  ...
  return (
    <div className="min-h-screen bg-[#0f0f0f] pb-24">
      <div className="h-44 md:h-56 w-full bg-gradient-to-br from-sky-900/80 via-[#0f0f0f] to-violet-900/40" />
      <div className="mx-auto max-w-[1200px] px-4 sm:px-6 -mt-20 pb-10">
        <div className="flex flex-col gap-6 md:flex-row md:items-end">
          <div className="h-32 w-32 shrink-0 overflow-hidden rounded-2xl border-4 border-[#0f0f0f] bg-black shadow-2xl md:h-40 md:w-40">
          ...
          <div className="flex-1 min-w-0">
            ...
            {user.employer ? (
              <Link
                href={`/agency/${user.employer.id}`}
                className="mt-3 inline-flex items-center gap-2 text-sm font-medium text-sky-400 hover:text-sky-300"
              >
                <Building2 className="h-4 w-4" />
                {user.employer.channel?.name ?? user.employer.name ?? user.employer.fullName}
              </Link>
            ) : null}
          </div>
        ...
        <section className="mt-12">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-white">Listings & tours</h2>
