...
export async function generateMetadata({
  ...
  const channel = await safeFindFirst(() =>
    prisma.channel.findUnique({
      where: { id: channelId },
      select: { name: true, description: true, profileImage: true, avatar: true },
    })
  );
  if (!channel) {
    return pageMetadata(resolved, `/channel/${channelId}`, {
      title: resolved === "ar" ? "القناة غير موجودة" : "Channel not found",
      noIndex: true,
    });
  }
  return pageMetadata(resolved, `/channel/${channelId}`, {
    ...
    ogImage: channel.profileImage ?? channel.avatar,
  });
}
