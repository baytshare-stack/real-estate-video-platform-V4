...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (sign?.error) {
        ...
        setTimeout(() => router.push(localizedPath("/login")), 2000);
        return;
      }
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const isAgentOrAgency = formData.role === "agent" || formData.role === "agency";
      router.push(localizedPath(isAgentOrAgency ? "/create-channel" : "/profile"));
      router.refresh();
    } catch {
      ...
      <div className="my-8 flex min-h-[calc(100vh-64px)] items-center justify-center p-4">
        <div className="w-full max-w-xl rounded-2xl border border-gray-800 bg-gray-900 p-6 shadow-2xl md:p-8">
          ...
          <p className="mt-8 text-center text-sm text-gray-400">
            <LocaleLink href="/login" className="font-medium text-blue-500 hover:text-blue-400">
              {t("auth", "backToSignIn")}
            </LocaleLink>
          </p>
        </div>
        ...
        {showGoogle ? (
          <div className="mb-6 space-y-3">
            <button
              ...
              onClick={() =>
                signIn("google", {
                  callbackUrl: `${localizedPath("/auth/post-login")}?from=register`,
                })
              }
