...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!user) {
    return pageMetadata(resolved, `/agency/${id}`, {
      title: resolved === "ar" ? "الوكالة غير موجودة" : "Agency not found",
      noIndex: true,
    });
  }
  const name = discoverDisplayName(user);
  return pageMetadata(resolved, `/agency/${id}`, {
    title: `${name} | ${resolved === "ar" ? "وكالة عقارية" : "Agency"}`,
    description:
      user.profile?.bio ??
      (resolved === "ar" ? "وكالة عقارية على ريال إستيت تي في" : "Real estate agency on Real Estate TV"),
