...
export default function StudioBookingsTable({ pollMs = 5000 }: Props) {
  ...
  const saveModify = async () => {
    if (!modifyId) return;
    const iso = toIso(modDate, modTime);
    if (!iso) return;
    ...
    <div className="space-y-4" dir={dir}>
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id} className="border-b border-white/[0.05] text-gray-200 last:border-0">
                    ...
                    <td className="px-4 py-3 align-top">
                      <span
                        className={`inline-block rounded-lg border px-2 py-0.5 text-xs font-semibold ${statusClass(r.status)}`}
                      >
                    ...
                    <td className="px-4 py-3 align-top">
                      <div className="flex flex-wrap gap-1.5">
                        {r.status === "PENDING" || r.status === "REJECTED" ? (
                          ...
                          >
                            <Check className="inline h-3.5 w-3.5" /> {t("studio", "bookings.accept")}
                          </button>
                        ) : null}
                        {r.status !== "REJECTED" ? (
                          <button
                            type="button"
                            disabled={busyId === r.id}
                            onClick={() => void patchBooking(r.id, { status: "REJECTED" })}
      ...
      {modifyId ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" dir={dir}>
          <div className="w-full max-w-sm rounded-2xl border border-gray-800 bg-gray-900 p-5 shadow-xl">
            <h4 className="mb-3 font-bold text-white">{t("studio", "bookings.modifyTitle")}</h4>
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2 sm:col-span-1">
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-gray-700 bg-gray-950 px-2 py-2 text-sm text-white"
                />
              </div>
              <div className="col-span-2 sm:col-span-1">
